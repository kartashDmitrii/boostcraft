...
if (document.querySelector('header')) {
  document.addEventListener('scroll', function () {
    if (window.screen.width > 767) {
      if (pageYOffset >= 55) {
        document.querySelector('.aside_user-menu .user-menu_search').style.top = '75px';
        document.querySelector('.aside_wrapper').style.maxHeight = '100vh';
        document.querySelector('.aside_menu').style.marginTop = "55px";
      } else {
        document.querySelector('.aside_user-menu .user-menu_search').style.top = "".concat(pageYOffset + 20, "px");
        document.querySelector('.aside_wrapper').style.maxHeight = "calc(100vh - 55px + ".concat(pageYOffset, "px");
        document.querySelector('.aside_menu').style.marginTop = "".concat(pageYOffset, "px");
      }
    }
  });
}
...
if (document.querySelector('.user-menu_mobile-sidebar-switcher') && document.querySelector('.aside')) {
  document.querySelector('.user-menu_mobile-sidebar-switcher').addEventListener('click', function () {
    if (window.screen.width <= 767) {
      ...
      var asideCloseFunc = function asideCloseFunc(event) {
        if (!event.target.closest('.aside') && !event.target.closest('.user-menu_mobile-sidebar-switcher')) {
          document.querySelector('.aside').classList.remove('active');
...
/* open/hide on tovar description   */


if (document.querySelector('.description-block')) {
  document.querySelectorAll('.description-block').forEach(function (desc_block) {
    desc_block.querySelector('.description-button').addEventListener('click', function () {
      var desc_text = desc_block.querySelector('.description-text');
      var desc_component_height = parseInt(window.getComputedStyle(desc_text).getPropertyValue('height'));

      if (desc_component_height === 66) {
        desc_text.style.height = "".concat(desc_text.scrollHeight, "px");
        desc_block.classList.add('active');
      } else {
        desc_text.style.height = '66px';
        desc_block.classList.remove('active');
      }
    });
  });
}
...
if (document.querySelector('.raid-calendar_calendar .calendar_dates')) {
  document.querySelectorAll('.calendar_dates').forEach(function (calendar) {
    ...
    var siema = new Siema({
      ...
      draggable: draggable,
      onChange: checkShadow
    });

    function checkShadow() {
      var wrapper = document.querySelector('.calendar_dates .dates_wrapper');
      document.querySelectorAll('.calendar_dates .dates_slider .dates_date').length - siema.perPage === siema.currentSlide ? wrapper.classList.remove('shadow-right') : wrapper.classList.add('shadow-right');
      siema.currentSlide === 0 ? wrapper.classList.remove('shadow-left') : wrapper.classList.add('shadow-left');
    }
...
if (document.querySelector('.cart-header')) {
  document.querySelectorAll('.user-menu_cart-btn').forEach(function (btn) {
...
if (document.querySelector('.user-menu_color-theme')) {
  document.querySelectorAll('.user-menu_color-theme').forEach(function (btn) {
    if (document.body.classList.contains('dark-theme')) {
      btn.classList.add('active');
    }

    btn.addEventListener('click', function () {
      document.querySelectorAll('.user-menu_color-theme').forEach(function (buttons) {
